...
class rl_model(object):
    ...
    def __init__(self):
        ...
        self.sub_car_cmd = rospy.Subscriber("~car_cmd", Twist2DStamped, self.correct_car_cmd, "lane_control")
        self.sub_img = rospy.Subscriber("~corrected_image/compressed", CompressedImage, self.store_image, "camera")
        self.pub_cor_car_cmd = rospy.Publisher("~corrected_car_cmd", Twist2DStamped, queue_size=1)
        rospy.on_shutdown(self.custom_shutdown)

        self.policy = DDPG()

        self.policy.load("DDPG123", "../model")
    ...
    def publishCmd(self, car_cmd_msg):
        self.pub_corr_car_cmd.publish(car_cmd_msg)
