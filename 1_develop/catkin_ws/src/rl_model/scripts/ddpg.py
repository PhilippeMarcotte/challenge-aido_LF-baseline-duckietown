...
class DDPG(object):
    ...
    def load(self, filename, directory, pickle=False):
        if pickle:
            import pickle
            with open('{}/{}_actor.pkl'.format(directory, filename)) as actor_pth, open('{}/{}_critic.pkl'.format(directory, filename)) as critic_pth:
                self.actor.load_state_dict(torch.load(actor_pth, map_location=device))
                self.critic.load_state_dict(torch.load(critic_pth, map_location=device))
        else:
